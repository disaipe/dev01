...
import type { CellFormulaValue, Color, FillPattern, Location, Workbook, Worksheet } from 'exceljs';

import type Handsontable from 'handsontable';
import type { CellRange } from 'handsontable';
  ...
  const syncData = () => {
    ...
    for (let row = 0; row < instance.value.countRows(); row++) {
      for (let col = 0; col < instance.value.countCols(); col++) {
        ...
        const merge = isMerged(xrow, xcol);
