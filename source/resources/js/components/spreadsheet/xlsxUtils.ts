...
import excel, { type Color, type FillPattern, type CellFormulaValue } from 'exceljs';
...
import type { SpreadsheetAlignment, SpreadsheetAlignmentClass, SpreadsheetBorder, SpreadSheetCellModifier } from '@/types';
...
type WorkSheetColumn = Partial<excel.Column>
type WorkSheetRow = Partial<excel.Row>
type WorkSheetExtended = {
    _columns?: WorkSheetColumn[]
    _rows?: WorkSheetRow[],
    _merges?: { [key: string]: excel.Range }
}
...
interface XlsxStore {
    ...
    worksheet?: excel.Worksheet & WorkSheetExtended;

    merges: excel.Location[],
...
export function configure(settings: Handsontable.GridSettings = {}): Handsontable.GridSettings {
    ...
    const syncSizes = () => {
        const toSync: Pick<Handsontable.GridSettings, "colWidths" | "rowHeights"> = {
            colWidths: undefined,
    ...
    return {
         ...
         */
        afterLoadData(sourceData: any[], initialLoad: boolean, source?: string) {
...
export function loadFromBuffer(buffer: ArrayBuffer) {
    ...
    workbook.xlsx.load(buffer).then(() => {
        const worksheet = workbook.getWorksheet(1) as excel.Worksheet & WorkSheetExtended;

        if (!worksheet) {
            console.warn('Failed to load worksheet from buffer');
            return;
        }
...
export function useHotTable(container: Ref<any>, { cellModifier }: { cellModifier?: SpreadSheetCellModifier } = {}) {
