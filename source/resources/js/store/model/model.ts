import type { BelongsTo, BelongsToMany, HasManyBy, ModelFields } from 'pinia-orm';
import type { ModelFieldRelation, ModelFieldSchema, ModelSchema, ModelKey } from '@/types';
...
export class Model extends ModelApi {
    ...
    static datetime(value: string): Datetime {
        return new Datetime(this.newRawInstance<any>(), value);
    }

    static date(value: string): Date {
        return new Date(this.newRawInstance<any>(), value);
    }
    ...
    $getName(): string {
        // @ts-ignore
        return this[this.$getDisplayField()] || `${this.$self().name}-${this.$getKey()}`;
    }
}
