...
import type { ModelSchema, HistoryRecord, ResponseBase } from '@/types';
...
export default class Api extends Repository {
    ...
    fetch(params = {}): Promise<{ response: Response, items: Model[] | undefined }> {
        return this.api()
            .post(this.baseURL(), params)
            .then((response: ResponseBase<Model[]>) => {
    ...
    remove(key: string | number | (string | number)[]): Promise<number[] | false> {
        return this.api()
    ...
    history(key: string | number): Promise<HistoryRecord[]> {
        return this.api()
    ...
    fetchRelatedModels() {
        return this.getRelatedModels().then((models: string[]) => {
            ...
            return this.api()
                ...
                .then((response: ResponseBase<{ [key: string]: Model[] }>) => {
                    if (response.status === 200) {
                        const { status, data } = response.data;
