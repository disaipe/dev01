...
.relative.h-full(ref='report')
  .flex.items-center.space-x-2.pb-4(ref='toolbar')
    el-select.shrink-0(
      ...
      el-option(
        v-for='companyOption of companies'
    ...
    el-select.shrink-0(
      ...
      el-option-group(
        ...
        el-option(
          v-for='reportTemplateOption of provider.report_templates'
          :label='reportTemplateOption.name'
          :value='reportTemplateOption.$getSingleKey()'
        )
...
function fetchReport() {
  ...
  api
    ...
    .then((response: InvoiceResponse) => {
      const message = 'Необработанная ошибка';
